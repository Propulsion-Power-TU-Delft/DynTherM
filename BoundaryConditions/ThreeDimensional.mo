...
package ThreeDimensional
  ...
  model thermal3D
    "Model to impose 3D distribution of heat flow rate and temperature"

    parameter Integer Nx=1 "Number of ports in x-direction";
    parameter Integer Ny=1 "Number of ports in y-direction";
    parameter Integer Nz=1 "Number of ports in z-direction";
    parameter Temperature T[Nx,Ny,Nz]=273.15*ones(Nx,Ny,Nz) "Temperature" annotation (Dialog(tab="Boundary conditions", enable=use_di_T));
    parameter HeatFlowRate Q[Nx,Ny,Nz]=1e3*ones(Nx,Ny,Nz) "Heat flow rate" annotation (Dialog(tab="Boundary conditions", enable=use_di_Q));
    ...
    CustomInterfaces.ThreeDimensional.HeatPort3D_A thermal(Nx=Nx,Ny=Ny,Nz=Nz)
  ...
  equation
    ...
    elseif use_in_T then
      thermal.ports.T = in_T_internal*ones(Nx,Ny,Nz);
    end if;

    if use_di_Q then
      thermal.ports.Q_flow + Q = zeros(Nx,Ny,Nz);
    elseif use_in_Q then
      thermal.ports.Q_flow + in_Q_internal/(Nx*Ny*Nz)*ones(Nx,Ny,Nz) = zeros(Nx,Ny,Nz);
    end if;
  ...
  model thermal_flux3D
    "Model to impose 3D distribution of heat flux and temperature"
    ...
    input Modelica.Units.SI.Temperature T[Nx,Ny,Nz]=273.15*ones(Nx,Ny,Nz) "Temperature" annotation (Dialog(tab="Boundary conditions", enable=use_T));
    input Modelica.Units.SI.HeatFlux phi[Nx,Ny,Nz]=1e3*ones(Nx,Ny,Nz) "Heat flux" annotation (Dialog(tab="Boundary conditions", enable=use_phi));
    ...
    CustomInterfaces.ThreeDimensional.HeatFluxPort3D_A thermal_flux(Nx=Nx,Ny=Ny,Nz=Nz)
      annotation (Placement(transformation(extent={{74,-16},{106,16}}),
          iconTransformation(extent={{74,-16},{106,16}})));
  ...
  equation
    ...
    if use_phi then
      thermal_flux.ports.phi + phi = zeros(Nx,Ny,Nz);
    end if;
