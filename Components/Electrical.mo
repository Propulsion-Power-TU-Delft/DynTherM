...
<p>[1] M. Macdonald. Early Design Stage Evaluation of Thermal Performance of Battery Heat Acuisition System of a Hybrid Electric Aircraft, 2020.</p>
  ...
  model CylindricalCell
    ...
    CylindricalCellPortion westPortion(
      ...
      annotation (Placement(transformation(
          ...
          rotation=90,
          origin={-28,0})));
    BoundaryConditions.ZeroDimensional.thermal internal(
      Q=-Q_vol*V_cell,
      use_Q=true,
      use_T=false)
  ...
  equation
    ...
    annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-120,-120},
      ...
      Documentation(info="<html>
<p>Equivalent circuit modeling irreversible heat generation by means of a resistor and a resistor-capacitor element.</p>
</html>"));
  end RC1;

  model PouchCell "Electro-thermal model of a pouch cell"
    ...
    Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor temperatureSensor annotation (Placement(transformation(extent={{6,34},{
              -6,46}})));
    TwoDimensional.PouchCellThermal2D thermal(
      redeclare model InPlaneMat = InPlaneMat,
      redeclare model CrossPlaneMat = CrossPlaneMat,
      W=W,
      H=H,
      t=t,
      dm=0,
      Tstart=Tstart,
      initOpt=initOpt,
      N=N) annotation (Placement(transformation(extent={{12,-28},{70,28}})));
    Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a Top annotation (Placement(
          transformation(extent={{34,48},{46,60}}), iconTransformation(extent={{-16,40},
              {-4,52}})));
    Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a Bottom annotation (
        Placement(transformation(extent={{34,-60},{46,-48}}),
                                                           iconTransformation(
            extent={{-16,-54},{-4,-42}})));
    ...
    CustomInterfaces.OneDimensional.HeatPort1D_A Left(Nx=N) annotation (
       Placement(transformation(
          extent={{-10,-5},{10,5}},
          rotation=90,
          origin={9,0}), iconTransformation(
          extent={{-30,-9},{30,9}},
          rotation=90,
          origin={-63,0})));
    CustomInterfaces.OneDimensional.HeatPort1D_A Right(Nx=N) annotation (Placement(transformation(
          extent={{-10,-5},{10,5}},
          rotation=90,
          origin={73,0}), iconTransformation(
          extent={{-30,-9},{30,9}},
          rotation=90,
          origin={45,0})));
    Modelica.Electrical.Analog.Interfaces.PositivePin p annotation (Placement(
  ...
  equation
    ...
    connect(Top,thermal. Top) annotation (Line(points={{40,54},{40.42,54},{40.42,17.36}},
                     color={191,0,0}));
    connect(Bottom,thermal. Bottom) annotation (Line(points={{40,-54},{40,-18.68},
            {40.42,-18.68},{40.42,-17.36}}, color={191,0,0}));
    connect(temperatureSensor.T, electrical.T) annotation (Line(points={{-6.6,40},
            {-86,40},{-86,25.45},{-68,25.45}},       color={0,0,127}));
    connect(temperatureSensor.T, cellBus.T) annotation (Line(points={{-6.6,40},
    ...
    connect(electrical.SoC, cellBus.soc) annotation (Line(points={{-45.5,-15.5},{
        ...
        extent={{-3,-6},{-3,-6}},
        horizontalAlignment=TextAlignment.Right));
    connect(electrical.Q, prescribedHeatFlow.Q_flow) annotation (Line(points={{-36.5,
            -15.5},{-36.5,-40},{-6,-40}},       color={0,0,127}));
    ...
    connect(Left, thermal.Left)
      annotation (Line(points={{9,0},{16.93,0}}, color={191,0,0}));
    connect(thermal.Right, Right)
      annotation (Line(points={{58.11,0},{73,0}}, color={191,0,0}));
    connect(temperatureSensor.port, thermal.Average) annotation (Line(points={{
            6,40},{28,40},{28,0},{40.42,0}}, color={191,0,0}));
    connect(prescribedHeatFlow.port, thermal.Average) annotation (Line(points={
            {6,-40},{28,-40},{28,0},{40.42,0}}, color={191,0,0}));
    annotation (
      ...
      experiment(StopTime=1200, __Dymola_Algorithm="Dassl"),
          ...
          coordinateSystem(preserveAspectRatio=false)),
      Documentation(info="<html>
<p>Model of a pouch cell coupling a first-order equivalent circuit model and a two-dimensional dynamic thermal model</p>
</html>"));
  end PouchCell;
