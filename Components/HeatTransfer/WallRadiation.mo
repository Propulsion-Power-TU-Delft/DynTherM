...
equation
  ...
  Q_absorbed + A*Paint.abs*(outlet.E_tb + outlet.E_td + outlet.E_tr) = 0;

  // Long-wave (infrared) radiative heat flux
  F_sky = (1 + cos(csi))/2;
  F_ground = (1 - cos(csi))/2;
  beta = sqrt(F_sky);
  Q_em_sky = A*environment.sigma*Paint.eps*F_sky*beta*(inlet.T^4 - environment.T_sky^4);
  Q_em_air = A*environment.sigma*Paint.eps*F_sky*(1 - beta)*(inlet.T^4 - environment.T_amb^4);
  Q_em_ground = A*environment.sigma*Paint.eps*F_ground*(inlet.T^4 - environment.T_ground_corr^4);
