...
model CircularPipeSideCond
  ...
  // Heat transfer
  model HTC =
    DynTherM.Components.HeatTransfer.HTCorrelations.InternalConvection.Gnielinski
      ...
      Pr=Pr,
      state=state,
      f = friction.f) annotation (choicesAllMatching=true);
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a port_a annotation (
      Placement(transformation(extent={{-40,30},{-20,50}}), iconTransformation(
          extent={{-40,30},{-20,50}})));
...
equation
  ...
  port_a.T = Medium.temperature(state);
  port_b.T = Medium.temperature(state);
  ...
  // Energy balance
  outlet.h_outflow = inStream(inlet.h_outflow) + thermalPort.Q_flow/inlet.m_flow + port_a.Q_flow/inlet.m_flow + port_b.Q_flow/inlet.m_flow;
  inlet.h_outflow = inStream(outlet.h_outflow) + thermalPort.Q_flow/inlet.m_flow + port_a.Q_flow/inlet.m_flow + port_b.Q_flow/inlet.m_flow;
