...
model RectangularPipe "Model of pipe with rectangular cross-section"
  ...
  // Heat transfer
  model HTC =
    DynTherM.Components.HeatTransfer.HTCorrelations.InternalConvection.Forrest
  ...
  // Pressure drop
  model DPC =
    DynTherM.Components.MassTransfer.DPCorrelations.Forrest (
      redeclare package Medium=Medium,
      Nx=N_cv,
      Ny=1,
      Dh=geometry.Dh,
  ...
  DynTherM.CustomInterfaces.FluidPort_B outlet(
    ...
    Xi_outflow(start=X_start)) annotation (Placement(transformation(extent={{80,
            ...
            110,10}})));
  DynTherM.CustomInterfaces.DistributedHeatPort_B thermalPort(Nx=N_cv, Ny=1) annotation (Placement(transformation(extent={{-40,8},
            {40,88}}), iconTransformation(extent={{-40,8},{40,88}})));
