...
model A320GroundCabin "Aircraft on ground, two temperature zones"
  ...
  Modelica.Blocks.Sources.Constant T_trim(k=373.15) annotation (Placement(
        ...
        rotation=0,
        origin={-20,90})));
  Modelica.Blocks.Sources.Constant m_trim_cab(k=0.0) annotation (Placement(
        transformation(
        extent={{10,10},{-10,-10}},
        rotation=180,
        origin={-90,-90})));
  ...
  inner Components.Environment environment(
    ...
    annotation (Placement(transformation(extent={{-66,-44},{-32,-10}})));
  Modelica.Blocks.Sources.Constant Mach_inf(k=0)
...
equation
  ...
  connect(PID_T_fd.y, A320.m_trim_cockpit) annotation (Line(points={{30,-37},{
          30,-19.25},{29.5714,-19.25},{29.5714,-17.5}},  color={0,0,127}));
  connect(A320.cockpitTemperature, PID_T_fd.u_m) annotation (Line(points={{37.8571,
          -17.5},{54,-17.5},{54,-48},{42,-48}},         color={0,0,127}));
  connect(m_trim_cab.y, A320.m_trim_cabin) annotation (Line(points={{-79,-90},
  ...
  connect(altitude.y,environment. altitude) annotation (Line(points={{-79,-50},
          {-74,-50},{-74,-33.8},{-66,-33.8}}, color={0,0,127}));
  connect(Mach_inf.y, environment.Mach_inf_di) annotation (Line(points={{-79,
          -10},{-74,-10},{-74,-27},{-66,-27}}, color={0,0,127}));
  annotation (
