...
model NH90CombinedFixedDuctHeat
  ...
  parameter Modelica.Units.SI.HeatFlowRate duct_ck=469 "Cockpit duct heat flow";
  parameter Modelica.Units.SI.HeatFlowRate duct_cb=170 "Cabin duct heat flow";
  ...
  BoundaryConditions.thermal cockpitOutOfPlenum(use_Q=true, Q=duct_ck)
    annotation (Placement(transformation(extent={{-12,44},{-6,48}})));
  BoundaryConditions.thermal cockpitIntoDuct(use_Q=true, Q=-duct_ck)
    annotation (Placement(transformation(extent={{-66,14},{-60,20}})));
  BoundaryConditions.thermal cabinOutOfPlenum(use_Q=true, Q=duct_cb)
    annotation (Placement(transformation(extent={{64,14},{70,18}})));
  BoundaryConditions.thermal cabinIntoDuct(use_Q=true, Q=-duct_cb)
    annotation (Placement(transformation(extent={{14,10},{20,14}})));
...
equation
  ...
  connect(cockpitOutOfPlenum.thermal, airbusCockpit.ductThermalPort)
    annotation (Line(points={{-8,46},{-28.8,46},{-28.8,47.8}}, color={191,0,0}));
  connect(cabinOutOfPlenum.thermal, airbusCabin.ductThermalPort) annotation (
      Line(points={{68,16},{53.4,16},{53.4,35.8}},                 color={191,0,
          0}));
  ...
  connect(airbusCabin.outletPort,evaporator_cb. inletPortRecirc) annotation (
      Line(points={{46.4,31.2},{62,31.2},{62,13},{53,13}}, color={0,0,0}));
  ...
  connect(evaporator_cb.plenumTemp, airbusCabin.plenumTemp) annotation (Line(
        points={{34,23},{28,23},{28,48},{33,48}}, color={0,0,127}));
  connect(evaporator_ck.thermalPort, cockpitIntoDuct.thermal) annotation (Line(
        points={{-45.4,20},{-50,20},{-50,17},{-62,17}}, color={191,0,0}));
  ...
  connect(cabinIntoDuct.thermal,evaporator_cb. thermalPort)
    annotation (Line(points={{18,12},{34.6,12},{34.6,14}}, color={191,0,0}));
  connect(extAirCb.outlet,evaporator_cb. inletPortFresh)
    annotation (Line(points={{72,10},{62,10},{62,9},{53,9}},   color={0,0,0}));
  connect(extAirCk.outlet,evaporator_ck. inletPortFresh) annotation (Line(
        points={{-14,14},{-14,15},{-27,15}},            color={0,0,0}));
  ...
  connect(airbusCabin.inletPort, evaporator_cb.outletPort) annotation (Line(
        points={{41.4,31.2},{24,31.2},{24,9},{35,9}}, color={0,0,0}));
