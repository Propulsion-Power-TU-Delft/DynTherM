...
// package Coolant = Modelica.Media.Water.ConstantPropertyLiquidWater;
  ...
  Components.TwoDimensional.ColdPlateCircularChannel1D Channel2D(
    ...
    Nt=3,
    N_channels=1)
    annotation (Placement(transformation(extent={{-50,28},{22,92}})));
  CustomInterfaces.Adaptors.heatFlowMultiplier1D heatFlowChannel2D(Nx=N_cv) annotation (Placement(transformation(
        extent={{-12,-6},{12,6}},
        rotation=90,
        origin={16,20})));
  ...
  Components.OneDimensional.CircularAsymmetricChannel1D Asymmetric(
    ...
    N_cv=N_cv,
    N_channels=1)
    annotation (Placement(transformation(extent={{-48,-96},{24,-24}})));
  CustomInterfaces.Adaptors.heatFlowMultiplier1D heatFlowAsymmetric(Nx=N_cv) annotation (Placement(transformation(
        extent={{-12,-6},{12,6}},
        rotation=90,
        origin={16,-10})));
...
equation
  connect(flow_source.outlet, Channel2D.inlet)
    annotation (Line(points={{-70,60},{-50,60}}, color={0,0,0}));
  connect(heatFlowChannel2D.distributed, Channel2D.BottomSurface) annotation (
      Line(points={{12.4,20},{3.28,20},{3.28,44.96}}, color={191,0,0}));
  connect(Channel2D.outlet, pressure_sink.inlet)
    annotation (Line(points={{22,60},{40,60}},                 color={0,0,0}));
  connect(Asymmetric.solid_surface_north, heatFlowAsymmetric.distributed)
    annotation (Line(points={{-39,-43.44},{-38.5,-43.44},{-38.5,-10},{12.4,-10}},
        color={191,0,0}));
  connect(flow_source1.outlet, Asymmetric.inlet) annotation (Line(points={{-68,-60},
          {-48,-60}},                     color={0,0,0}));
  connect(Asymmetric.outlet, pressure_sink1.inlet) annotation (Line(points={{24,-60},
          {40,-60}},                        color={0,0,0}));
  connect(heatFlowChannel2D.single, heatSensorChannel2D.port_a)
    annotation (Line(points={{19.6,20},{30,20}}, color={191,0,0}));
  connect(heatSensorAsymmetric.port_b, FixedTemperature.thermal) annotation (
      Line(points={{50,-10},{82,-10},{82,4},{83.3333,4}}, color={191,0,0}));
  connect(heatFlowAsymmetric.single, heatSensorAsymmetric.port_a)
    annotation (Line(points={{19.6,-10},{30,-10}}, color={191,0,0}));
  connect(heatSensorChannel2D.port_b, FixedTemperature.thermal) annotation (
      Line(points={{50,20},{84,20},{84,4},{83.3333,4}}, color={191,0,0}));
